...
{
    public int SecondsPerGame = 180;                  // time per round/turn
    ...
    void Update()
    {
        double remainingTime = (int)(PhotonNetwork.time - StartTime) % SecondsPerGame;
        remainingTime = (SecondsPerGame - remainingTime);
        if (TimeLeft != null)
            TimeLeft.GetComponent<Text>().text = "Time Left : " + (int)remainingTime / 60 + ":" + remainingTime % 60;
        if ((int)(PhotonNetwork.time - StartTime) % SecondsPerGame == 0 && !once)
        {
            once = true;
            GameObject.Find("NetworkManager").GetComponent<NetworkManager>().PopGameOver();
            TimeLeft.GetComponent<Text>().text = "";
            this.GetComponent<InRoomRoundTimer>().enabled = false;
        }
    ...
    //    double remainingTime = SecondsPerGame - (elapsedTime % SecondsPerGame);
    //    int turn = (int)(elapsedTime / SecondsPerGame);
